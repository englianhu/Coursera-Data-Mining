...
unzip(paste0('data/', destfile), list = TRUE) %>% kable #%>% formattable(Length = normalize_bar('blue', 0.2), Date = color_tile('white', 'orange'))
```
...
summ <- llply(as.list(datafiles), file.info) %>% rbind_all #%>% formattable(., list(size=color_bar('pink', 0.5)))
dt <- dTable(summ, sPaginationType = "full_numbers", iDisplayLength = 6, sScrollX = "100%")
dt$show('inline', include_assets = TRUE, cdn = TRUE)
```
...
## 5.3 Speech and Blooper
  ...
  I faced few issues during preparation of this assignment.
  - Tried to refer to [Add sparkline graph to a table](http://stackoverflow.com/questions/32841221/add-sparkline-graph-to-a-table)^[There has [**formattable** package](https://github.com/renkun-ken/formattable) and also an example of [mixing **sparklines** with formattable](https://github.com/renkun-ken/formattable/issues/10#issuecomment-110864675)] and applied it on my dataset but formattable doesn't work.
  - 
