...
pkgs <- c('jsonlite', 'arules', 'arulesViz', 'readr', 'knitr', 'stringr', 'stringi', 'dplyr', 'purrr', 'magrittr', 'formattable', 'DT', 'lubridate', 'googleVis')
l_ply(pkgs, require, quietly = TRUE, character.only = TRUE)
rm(pkgs)
...
transDat <- read.transactions('https://raw.githubusercontent.com/englianhu/Coursera-Data-Mining/master/4%20Pattern%20Discovery%20in%20Data%20Mining/data/categories.txt', format = 'single', cols = c(1, 2), sep = ';')

transDat2 <- read.transactions('https://raw.githubusercontent.com/englianhu/Coursera-Data-Mining/master/4%20Pattern%20Discovery%20in%20Data%20Mining/data/categories.txt', format = 'basket', sep = ';')
```
...
```{r item2a}
inspect(transDat[1:100]) # view the observations
length(transDat) # get number of observations
size(transDat[1:100]) # number of items in each observation
...
```{r apriori-a}
# Get the rules
rules <- apriori(transDat, parameter = list(supp = 0.001, conf = 0.8))
...
```{r apriori-b}
# Get the rules
rules <- apriori(transDat2, parameter = list(supp = 0.001, conf = 0.8))
